...
/**
 * The pack reveal: `BareModal` supplies the shell — the page darkens and stops responding while the dialog itself
 * has no surface — and this component owns what floats inside it. Five face-down cards sit on the darkened page,
 * clicking one flips it in 3-D with a burst of light, and the Continue button appears at the bottom of the dialog
 * only once every card is face up.
 *
...
export const PackRevealModal: React.FC<PackRevealModalProps> = ({ open, cards, onClose }) => {
  ...
  return (
    // Esc and the backdrop only close the dialog once nothing is left face-down, so a stray click cannot skip the
    // reveal; until then the Continue button is the way out (that is the whole meaning of `dismissable` here).
    <BareModal
      open={open}
      onClose={onClose}
      dismissable={allRevealed}
      maxWidth="lg"
      fullWidth
      className="pack-reveal-dialog"
      ariaLabel="Pack reveal"
    >
